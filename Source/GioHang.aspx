...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" Runat="Server">
    <section class="container-page py-10 lg:py-16">
        <div class="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            ...
            <span class="self-start rounded-full bg-[var(--surface)] px-5 py-3 text-sm font-black text-[var(--ink-soft)]"><asp:Literal ID="litCartCount" runat="server">0</asp:Literal> sản phẩm</span>
        </div>
        ...
        <div class="grid gap-8 lg:grid-cols-[1fr_24rem]">
            <div class="space-y-4">
                <asp:Repeater ID="rptCartItems" runat="server">
                    <ItemTemplate>
                        <article class="surface-panel flex flex-col gap-5 p-4 sm:flex-row sm:items-center sm:p-5" data-cart-item>
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                <div class="mt-4 flex flex-wrap items-center gap-4">
                                    ...
                                    <div class="price-text text-lg"><%# Eval("ThanhTien", "{0:N0}đ") %></div>
                                </div>
                            </div>
                            <button type="button" onclick="removeItem(<%# Eval("MaSP") %>)" class="self-start rounded-full border border-[var(--line)] px-4 py-2 text-xs font-black text-[var(--danger)] hover:bg-red-50 sm:self-center">Xóa</button>
                        </article>
                    </ItemTemplate>
                </asp:Repeater>
            ...
            <aside>
                <div class="surface-panel sticky top-28 p-6 sm:p-8">
                    ...
                    <a href="Checkout.aspx" class="btn-primary mt-8 w-full py-4">Tiến hành thanh toán</a>
    ...
    <script>
        function updateQty(maSP, delta, btn) {
            ...
            fetch('CartHandler.ashx?action=update&maSP=' + maSP + '&qty=' + newQty)
                ...
                .then(data => {
                    ...
                    else alert(data.message || 'Không thể cập nhật giỏ hàng.');
                });
        }
        ...
        function removeItem(maSP) {
            ...
            fetch('CartHandler.ashx?action=remove&maSP=' + maSP)
                ...
                .then(data => {
                    if (data.success) location.reload();
                    else alert(data.message || 'Không thể xóa sản phẩm.');
                });
        }
    </script>
</asp:Content>
